...
void draw() {
  ...
  if (showBestEachGen) {//show the best of each gen
    if (!genPlayerTemp.dead) {//if current gen player is not dead then update it
      ...
      upToGen ++;
  // if (genPlayerTemp.score != 0 && genPlayerTemp.score > highscore){
...
void writeInfo() {
  ...
  if (showBestEachGen) { //if showing the best for each gen then write the applicable info
    ...
    //text("Framerate: " + frameRate,width /2, height -30);
    textSize(20);
    int x = 580;
    ...
    textAlign(LEFT);
    text("Highscore: "+pop.bestScore, width/2 - 140, height-30);
    textAlign(RIGHT);

    //text("Frame Rate: " + frameRate , width /2 , height-30);
